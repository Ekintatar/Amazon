...
"Go to log in page"
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, "nav-link-accountList"))).click()

"Email and password fields are selected and filled with the info from the 'account.py' file"
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, "ap_email"))).send_keys(account.mail)
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, "ap_password"))).send_keys(account.password)

"Logged in"
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, "signInSubmit"))).click()

"Confirmed the the log in is successful"
accName = WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.CLASS_NAME, "nav-a.nav-a-2.nav-truncate"))).text.find(account.name)
...
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, "twotabsearchtextbox"))).send_keys("samsung"+ Keys.ENTER)

"Confirm the result page is contains the desired product"
searchText = WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.CLASS_NAME, "a-color-state.a-text-bold")))
assert searchText.text.find('samsung') > -1 , ("samsung could not be searched")

"Go to second second page"
WebDriverWait(browser, 3).until(ec.presence_of_all_elements_located((By.CSS_SELECTOR, '.a-section.s-border-bottom .a-normal')))[0].click()


"Click on the third product on the page"
WebDriverWait(browser, 3).until(ec.presence_of_all_elements_located((By.CSS_SELECTOR, '.a-size-medium.a-color-base.a-text-normal')))[2].click()


"Get the products name"
productName = WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, 'productTitle'))).text.split('\n')[0]
...
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.ID, 'WLHUC_viewlist'))).click()

"Delete the newly added product"
WebDriverWait(browser, 3).until(ec.presence_of_element_located((By.CSS_SELECTOR, '#a-autoid-7 > span > input'))).click()
